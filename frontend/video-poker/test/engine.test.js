...
test('minPair: kings makes jacks and queens pay nothing (Triple Bonus Poker)', () => {
  const pt = { family: 'standard', quadRule: 'rank-tier', minPair: 'kings', rows: [] };
  const C = E.CATEGORY;
  assert.strictEqual(E.resolveCategory(hand('JS JH 7D 4C 2H'), pt), C.NOTHING);
  assert.strictEqual(E.resolveCategory(hand('QS QH 7D 4C 2H'), pt), C.NOTHING);
  assert.strictEqual(E.resolveCategory(hand('KS KH 7D 4C 2H'), pt), C.KINGS_OR_BETTER);
  assert.strictEqual(E.resolveCategory(hand('AS AH 7D 4C 2H'), pt), C.KINGS_OR_BETTER);
  assert.strictEqual(E.resolveCategory(hand('JS JH 7D 7C 2H'), pt), C.TWO_PAIR);
});
